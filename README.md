...
| Key | Default | Description |
|-----|---------|-------------|
| `enabled` | `true` | Read the user-level Codex `config.toml` and merge its MCP servers into `mcpServers`; `false` disables only MCP-server import |
