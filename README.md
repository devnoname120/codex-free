...
In native-tunnel mode, Codexrr listens only on `127.0.0.1`, starts OpenAI's official runtime-only tunnel client, and supervises it for the lifetime of the server. The tunnel client makes outbound HTTPS requests to OpenAI and forwards only tunnel traffic to the loopback MCP endpoint. A conventional externally managed tunnel remains available as an alternative.
...
There is no server URL to enter in this mode. OpenAI routes the selected tunnel to the supervised client, and that client alone can reach Codexrr's loopback MCP listener. The startup banner includes the local diagnostics UI; it is not exposed through the tunnel.
...
- **Native OpenAI tunnel is outbound-only**: Codexrr binds its MCP listener to loopback and supervises OpenAI's official runtime-only tunnel client. Startup fails unless the client can read the tunnel metadata, probe the local MCP endpoint, and complete a control-plane poll. The child is stopped when Codexrr exits or when either side fails.
- **Verified tunnel-client installation**: the managed client is pinned to a specific official release, downloaded over HTTPS, checked against that release's `SHA256SUMS.txt`, extracted by exact filename under size limits, installed atomically with private permissions, and hash-checked on subsequent starts. Set `clientPath` to opt out of managed installation while retaining compatibility checks.
- **Tunnel secrets are references, not config values**: `openaiTunnel.apiKeyRef` accepts only `env:NAME` or `file:/path`; literal API keys are rejected. Use a restricted runtime key with Tunnels **Read** + **Use**, not an admin key. Private key-file permissions are enforced on Unix.
