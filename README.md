...
flowchart LR
    ...
    DiffRefs[("Git refs/codex-free/diff\nproject-open + last-diff")]
    DiffUI["MCP App diff card\nui://codex-free/diff/v2/mcp-app.html"]
...
- **`apply_patch` takes a JSON string.** In Codex it is a *freeform* tool whose entire body is the raw patch. MCP has no freeform tools, so the patch goes in an `input` string parameter. The patch grammar and verified-then-sequential execution semantics are otherwise the same: all hunks are checked before the first write, but a later filesystem error can leave earlier verified operations applied.
- **`exec_command` runs with plain pipes, not a PTY.** Codex's own `tty` parameter documents pipes as the default, so ordinary commands behave the same; `tty: true` is rejected rather than silently ignored. Programs that only enable interactive behaviour when attached to a terminal will act as if piped. As in Codex, `shell` is a shell-type hint, not an executable path: only the basename is considered, and an unrecognized or unavailable shell uses Codex's ultimate platform fallback (`/bin/sh` on POSIX, `cmd.exe` on Windows).
...
After publication, the result is an ordinary project file. Git, `glob`, `tree`, diff tools and normal deletion provide its catalogue and lifecycle; Codex Free does not maintain a second artifact database or TTL.
...
```text
refs/codex-free/diff/<project-hash>/<conversation-hash>/project-open
refs/codex-free/diff/<project-hash>/<conversation-hash>/last-diff
```

The raw conversation identifier is never written. The refs survive MCP reconnects and Codex Free restarts. Generic MCP clients receive transport-local in-memory checkpoints instead. Each conversation/project pair retains only its current two referenced snapshots; superseded synthetic commits are ordinary Git-GC candidates. To inspect or remove current refs manually, use `git for-each-ref refs/codex-free/diff/` and `git update-ref -d <ref>`. Removing both refs resets that owner to the current scoped state on its next project call. Existing `refs/codex-free/review/.../project-open` and `.../last-review` refs are copied lazily into the new namespace and retained so older installations can still read them.

Codex Free also advertises the standard MCP Apps extension and serves a self-contained resource at `ui://codex-free/diff/v2/mcp-app.html`. Compatible ChatGPT developer connectors render `show_diff` as a file, statistic and patch card from component-only result metadata. No separate web service or public app publication is needed; clients that ignore MCP Apps metadata receive only the concise ordinary text result. The component is model-visible but is not granted app-side tool access. The card remains independently interactive while the model continues, and its overall disclosure, expanded file diffs, and larger-file-list state are persisted as private ChatGPT widget state so an iframe remount restores them. The former `io.github.hypnguyen1209/review` metadata key and both historical `ui://codex-free/review/...` resource URIs remain readable so existing cards can remount; current `show_diff` results emit only the diff-named metadata. The card updates at the `show_diff` tool-call boundary—it is not a continuous filesystem watcher. Cursor advancement completes before the result is returned and never waits for widget interaction.
...
With `"mode": "direct"`, each upstream tool becomes a `BridgedTool` named `<server>__<tool>` (sanitized to `[A-Za-z0-9_]`, so `remote-exec` becomes `remote_exec__exec`). Calls use the tool's stored **raw upstream name**, not the downstream identifier. Input/output schemas, title, icons, `_meta`, and every annotation field supplied upstream are preserved in downstream `tools/list`; omitted safety hints are materialized with the MCP defaults so the downstream descriptor is complete. Text, images, structured content, error state, and result metadata pass through on calls. A downstream name colliding with a native or previously registered tool is skipped with a warning.
...
- **Namespaced diff state inside Git**: ChatGPT diff checkpoints are exactly two refs per conversation/project pair under `refs/codex-free/diff/`. Synthetic snapshots contain only the selected project path, are built through a temporary index, and never modify the real index or working tree. Generic MCP-client checkpoints are in memory only. The namespace grows with the number of distinct persistent conversation/project pairs; the diff section documents inspection, compatibility migration, and manual removal.
