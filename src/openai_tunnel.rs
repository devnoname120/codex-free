...
async fn validate_managed_install(
    ...
    {
        bail!("managed OpenAI tunnel installation manifest does not match this Codexrr build");
    }
    let actual_hash =
...
async fn install_managed_client(
    ...
    if archive_hash != asset.archive_sha256 {
        bail!("OpenAI tunnel runtime archive does not match the hash pinned by this Codexrr build");
    }
...
async fn validate_client(path: &Path, exact_version: Option<&str>) -> anyhow::Result<()> {
    ...
    {
        bail!(
            "OpenAI tunnel client is incompatible with Codexrr's supervised runtime mode: {}",
