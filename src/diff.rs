...
const DIFF_REF_ROOT: &str = "refs/codex-free/diff";
const LEGACY_REVIEW_REF_ROOT: &str = "refs/codex-free/review";
const LEGACY_PROJECT_SCOPE_HASH_DOMAIN: &[u8] = b"codex-free/review-project/v1\0";
const SYNTHETIC_IDENTITY_NAME: &str = "Codex Free Diff";
const SYNTHETIC_IDENTITY_EMAIL: &str = "diff@codex-free.local";
...
fn hash_path(path: &Path) -> String {
    ...
    // Preserve the opaque project key so existing checkpoint refs can be copied
    // into the renamed namespace without scanning or exposing project paths.
...
async fn create_snapshot(
    ...
    let commit = git_checked(
        ...
        vec![
            ...
            OsString::from(tree),
            OsString::from("-m"),
            OsString::from("Codex Free diff snapshot"),
        ],
        &environment,
    )
    .await?;
