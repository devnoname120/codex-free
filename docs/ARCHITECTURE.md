...
Codexrr continues to serve its existing Streamable HTTP endpoint, while the
...
Codexrr watches the HTTP server, tunnel child, `SIGINT`, and `SIGTERM`
...
| Native tunnel never becomes ready | Check the local diagnostics UI and logs. `/readyz` covers local MCP probing, while Codexrr additionally requires tunnel metadata and one successful control-plane poll; the runtime key needs Tunnels **Read** + **Use**. |
